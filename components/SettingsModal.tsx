...
import {
  ...
  ToggleRight, ToggleLeft, CheckSquare, Square, Edit, FileJson, Sliders, RefreshCcw, ExternalLink, AlertTriangle, ChevronLeft, Save, Plus, FileCode, Languages, RotateCcw
} from 'lucide-react';
import { testConnection, googleTranslateFree } from '../services/apiService';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, settings, onSave, initialTab }) => {
  ...
  const [nameOriginal, setNameOriginal] = useState<string | null>(null);
  ...
  const handleTranslateName = async () => {
    if (!localSettings.userName.trim() && nameOriginal === null) return;

    if (nameOriginal !== null) {
      handleChange('userName', nameOriginal);
    ...
    try {
      const hasArabic = /[\u0600-\u06FF]/.test(localSettings.userName);
      const target = hasArabic ? 'en' : 'ar';
      const translated = await googleTranslateFree(localSettings.userName, target);
      setNameOriginal(localSettings.userName);
      handleChange('userName', translated);
    } catch (e) {
  ...
  const handleTranslatePersona = async () => {
    if (!localSettings.userPersona.trim() && personaOriginal === null) return;
    ...
    try {
      const hasArabic = /[\u0600-\u06FF]/.test(localSettings.userPersona);
      const target = hasArabic ? 'en' : 'ar';
      const translated = await googleTranslateFree(localSettings.userPersona, target);
      setPersonaOriginal(localSettings.userPersona);
                             ...
                             <div className="flex-1 w-full space-y-4">
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        <input
                                            className="flex-1 bg-black border border-zinc-800 p-3 text-zinc-200 focus:border-orange-500/50 outline-none text-sm"
                                        ...
                                        <button
                                            ...
                                            disabled={nameTranslating || (!localSettings.userName.trim() && nameOriginal === null)}
                                            className="px-3 bg-black border border-zinc-800 hover:border-zinc-600 text-zinc-400 hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5"
                                            title={nameOriginal ? "Restore Original" : "Translate (Auto Detect)"}
                                        >
                                            {nameTranslating ? <Loader2 size={14} className="animate-spin" /> : nameOriginal ? <RotateCcw size={14} /> : <Languages size={14} />}
                                        </button>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <button
                                            ...
                                            disabled={personaTranslating || (!localSettings.userPersona.trim() && personaOriginal === null)}
                                            className="absolute top-2 right-2 px-2 py-1 bg-zinc-900/80 border border-zinc-700 hover:border-zinc-600 text-zinc-400 hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1.5 text-[10px] uppercase font-bold tracking-wider rounded"
                                            title={personaOriginal ? "Restore Original" : "Translate (Auto Detect)"}
                                        >
                                            {personaTranslating ? <Loader2 size={12} className="animate-spin" /> : personaOriginal ? <RotateCcw size={12} /> : <Languages size={12} />}
